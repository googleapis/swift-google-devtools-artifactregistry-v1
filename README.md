...
Add `swift-google-devtools-artifactregistry-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-devtools-artifactregistry-v1.git --from 0.1.0-preview
```

Then add `GoogleCloudArtifactRegistryV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleCloudArtifactRegistryV1 <target-name> --package swift-google-devtools-artifactregistry-v1
```
